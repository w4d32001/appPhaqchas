...
export default function Booking(props: BookingProps) {
  ...
  const [firstWeekStart] = useState(currentWeekStart);
  ...
  return (
    <div className="w-full px-2">
      ...
      >
        {!isSameWeek(currentWeekStart, firstWeekStart, { weekStartsOn: 1 }) && (
          <button
            onClick={goToPreviousWeek}
            className="text-white rounded-xl bg-orange-600 hover:bg-orange-900/90 py-2 transition-colors px-4 text-xl flex items-center justify-between"
          >
        ...
        <button
          ...
          className="text-white rounded-xl bg-orange-600 hover:bg-orange-900/90 py-2 transition-colors px-4 text-xl flex items-center justify-between"
        >
          <span className="hidden md:block">Siguiente semana</span>{" "}
